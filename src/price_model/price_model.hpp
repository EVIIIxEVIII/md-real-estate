...
class PriceModel {
    ...
    private:
        ...
        std::array<double, NF> stds;
        std::array<double, NF> means;

        SplitData split_data(LoadedData<PriceModel::F> data);

        LoadedData<F> load_data(std::string data_path);
        ...
        void save_vector_binary(const std::string& filename, const Eigen::VectorXd& vec);
