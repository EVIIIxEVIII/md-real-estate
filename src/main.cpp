...
int main(void)
{
	...
	SetTargetFPS(60);
    Font font = LoadFontEx("resources/DejaVuSans-Bold.ttf", 24, nullptr, 0);
    ...
    inputs.emplace_back(std::make_unique<ui::Input>("Total area", 180, total_area));
    inputs.emplace_back(std::make_unique<ui::Input>("Number of rooms", 180, number_of_rooms));
    inputs.emplace_back(std::make_unique<ui::Input>("Number of floors", 180, number_of_floors));
    inputs.emplace_back(std::make_unique<ui::Input>("Bathrooms", 180, bathrooms));
    inputs.emplace_back(std::make_unique<ui::Input>("Floor", 180, floor));
    ...
    const float init_pos_inputs = center(inputs.size() * 200);
    const float init_pos_selects = center(selects.size() * 200);
